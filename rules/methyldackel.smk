...
rule methyldackel_extract_se:
    input:
        bam = RESULT_DIR / '01_bwameth' / 'se' / '{sample}.sorted.bam',
        bam_index = RESULT_DIR / '01_bwameth' / 'se' / '{sample}.sorted.bam.bai',
    ...
    output:
        ...
        RESULT_DIR / '02_methyldackel' / 'se' / '{sample}_CpG.bedGraph',
    params:
...
rule methyldackel_extract_pe:
    input:
        bam = RESULT_DIR / '01_bwameth' / 'pe' / '{sample}.sorted.bam',
        bam_index = RESULT_DIR / '01_bwameth' / 'pe' / '{sample}.sorted.bam.bai',
        reference = config['reference']['fasta'],
    output:
        # NOTE: {sample}_CpG.meth.bedGraph will automatically use --fraction option,
        # {sample}_CpG.counts.bedGraph will automatically use --counts option, and
        # {sample}_CpG.logit.bedGraph will automatically use --logit option.
        RESULT_DIR / '02_methyldackel' / 'pe' / '{sample}_CpG.bedGraph',
    params:
        # Optional parameters. Omit if unneeded.
        extra = MEP['extra'],
        # Minimum MAPQ threshold to include an alignment.
