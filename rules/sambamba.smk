...
rule sambamba_sort_se:
    ...
    output:
        RESULT_DIR / '01_bwameth' / 'se' / '{sample}.sorted.bam'
    params:
        extra = SSS['extra'],
    ...
    log: 'logs/sambamba_sort_se/{sample}.log'
    benchmark: 'benchmarks/sambamba_sort_se/{sample}.benchmark'
    wrapper:
...
rule sambamba_sort_pe:
    ...
    output:
        RESULT_DIR / '01_bwameth' / 'pe' / '{sample}.sorted.bam'
    params:
        extra = SSP['extra'],
        # Sets an upper bound for used memory. However, this is very approximate.
        # Default memory limit is 512 MiB. Increasing it will allow to make chunk
    ...
    log: 'logs/sambamba_sort_pe/{sample}.log'
    benchmark: 'benchmarks/sambamba_sort_pe/{sample}.benchmark'
    wrapper:
        'http://dohlee-bio.info:9193/sambamba/sort'
...
rule sambamba_index_se:
    input:
        RESULT_DIR / '01_bwameth' / 'se' / '{sample}.sorted.bam'
    output:
        RESULT_DIR / '01_bwameth' / 'se' / '{sample}.sorted.bam.bai'
    threads: config['threads']['sambamba_index_se']
    log: 'logs/sambamba_index_se/{sample}.log'
    benchmark: 'benchmarks/sambamba_index_se/{sample}.benchmark'
    wrapper:
...
rule sambamba_index_pe:
    input:
        RESULT_DIR / '01_bwameth' / 'pe' / '{sample}.sorted.bam'
    output:
        RESULT_DIR / '01_bwameth' / 'pe' / '{sample}.sorted.bam.bai'
    threads: config['threads']['sambamba_index_pe']
    log: 'logs/sambamba_index_pe/{sample}.log'
    benchmark: 'benchmarks/sambamba_index_pe/{sample}.benchmark'
    wrapper:
        'http://dohlee-bio.info:9193/sambamba/index'
